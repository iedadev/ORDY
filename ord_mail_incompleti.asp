...
s =  "SELECT DISTINCT numord, DataPagamento, DataOrdine, DataCompleto, lc FROM ORD_Richieste  WHERE STATOORDINE =0 AND DATAPAGAMENTO IS NOT NULL AND ARCHIVIO3 =0" 
...
<a href="mailto:<%= rsm("Emacli") %>?bcc=segreteria@motherwords.it;it@motherwords.it&subject=LEARNING CENTER <%= rs("LC") %> ORDINE <%= rs("numord") %> INCOMPLETO&body=
Ciao%0A
Per il tuo ordine Nr <%= rs("numord") %> abbiamo ricevuto il pagamento ed il tuo ordine è stato inoltrato.%0A
...
Grazie,%0A Lorenza %0A%0A">
                                                 <img src="images/email.png" width="32" height="32" title="Invia mail"></a>
															...
															sc = "SELECT COUNT(*) as nrord FROM ORD_COMUNICAZIONI WHERE LC ='" & rs("LC") & "' AND STATOORDINE =2 AND INCOMPLETO = 0 AND NRORDINE =" & rs("numord") 
															...
															If  rsc("nrord") >0 then
															' prevedere update del campo incompleto =1 nella tabella' 
															%>
															<a href="ord_mail_invio_incompleti.asp?ORDINE=<%= rs("numord") %>&LC=<%= rs("LC") %>"><button type="button" class="btn btn-sm btn-primary">Conferma Invio</button></a>
															<%else%>
															 <button type="button" class="btn btn-sm btn-success" disabled>Mail Inviata</button>
                                                             <a href="ord_mail_invio_incompleti_archivio.asp?ARCHIVIO=1&ORDINE=<%= rs("numord") %>&LC=<%= rs("LC") %>"><button type="button" class="btn btn-sm btn-warning">Archivia</button></a>
															<%end if
