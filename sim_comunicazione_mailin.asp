                            ...
                            <div class="navbar navbar-inner block-header">
                            	<legend><%=response.write (titolo93)%> 

                                     <%
                                        
                                        'mail da leggere
										iduser=session("id_usr")
                                        ss0 = "SELECT COUNT(*) as TOTMAIL FROM SIM_Comunicazioni WHERE DESTINATARIO =" & iduser & " AND STATOLETTURA = 0"
                                        Set rs0 = dbConn.Execute(ss0)
                                        
                                        'mail lette

                                        'ss2 = "SELECT COUNT(*) as TOTMAILR FROM SIM_Comunicazioni WHERE DESTINATARIO =" & iduser & " AND STATOLETTURA = 1"
                                        'Set rs2 = dbConn.Execute(ss2)

                                     %>
 
                                    <h6><img src="images/email.png" width="16" height="16"> (<%=rs0("Totmail")%>&nbsp;<%=response.write (titolo85)%>)
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="sim_comunicazione_archiviomail.asp"> <img src="images/email_open.png" width="16" height="16"></a></legend> Archivio mail</h6></div><br>
                               
                                  <%if rs0("Totmail")= 0 Then%>
                                    
                                <div class="container span11">
                                      <div class="alert alert-success">
                                        <strong>Success!</strong> Non ci sono mail da leggere.Buona giornata.
                                      </div>
                                </div>
                                <%Else%>
                                <table span="3" cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
										...
										<tbody>
											<%
                                                ...
                                                sss = "SELECT * FROM SIM_Comunicazioni WHERE DESTINATARIO =" & iduser & " AND STATOLETTURA = 0"
                                                ...
                                                'Response.Write rs("POS")
													...
													<% End If %>
														...
														<td><%    
                                                            ...
                                                            <td><%= rs("TESTO") %></td>
                                                             <td>
                                                                 <a href="sim_comunicazione_mail_conferma.asp?IDCOM=<%= rs("IDCOM") %>"><img src="images/email_read.png" width="32" height="32" title="<%=response.write (titolo86)%>"></a>
                                                             </td>
												       </tr>
													<%
													rs.MoveNext
